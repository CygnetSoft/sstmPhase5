...
function GetMainCoursesList(ctype) {
    ...
    Ajaxhelper.get(getSubCoursesListUrl, params, onSuccessGetMainCoursesList, null, null);
...
function InittblMainCourses() {
    ...
    if (isAuthorized) {
        $('#tblMainCourses > tbody > tr').on('click', '.btnSubEditCourse', function (e) {
            e.preventDefault();
            var Id = $(this).closest('tr').attr('id');
            
            OpenAddOrUpdateSubCourseModal(Id);
        });

        $('#tblMainCourses > tbody > tr').on('click', '.btnSubDeleteCourse', function (e) {
            e.preventDefault();
            var Id = $(this).closest('tr').attr('id');
    ...
    $('#tblMainCourses > tbody > tr').on('click', '.btnSubCourse', function (e) {

        e.preventDefault();
        var Id = $(this).closest('tr').attr('id');
        
        if (userRole === 'Trainer' || userRole === 'Print Incharge')
            window.location.href = gotoSharedCoursesList.replace('__id__', Id);
        else {
            if (CourseType == "other")
...
function OpenAddOrUpdateSubCourseModal(Id) {
    
    Ajaxhelper.post(getSubCourseByIdUrl, { Id: Id, type: CourseType, mainCourseId: MainCourseId }, onSuccessGetMainCourseById, null, null);

    function onSuccessGetMainCourseById(data) {

        $('#divAddOrEditSubCourseModal').html(data);
        $('#AddOrEditSubCourseModal').modal('show');
    }
}