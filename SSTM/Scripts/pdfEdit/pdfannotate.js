...
PDFAnnotate.prototype.enableAddArrow = function (onDrawnCallback = null) {
    ...
    PDFAnnotate.prototype.savePdf = function (fileName) {
        $(".savepdf").attr('disabled','disabled');
        var inst = this;
            ...
            if (index === inst.fabricObjects.length - 1) {
                ...
                reader.onloadend = function () {
                   ...
                   base64String.substr(base64String.indexOf(', ') + 1));
                    ...
                    {
                        alert("Enter Mark data after save proceed");
                        $(".savepdf").removeAttr('disabled','disabled');
                        return;
                    }
                    Ajaxhelper.post(savepdf, { pdf: base64String,
                        filename: fileName,
        ...
        $('#tblstudentmark > tbody  > tr').each(function (index, tr) {
            if ($(this).closest('tr').find('td:eq(1)').find('input[type="text"]').val() != "") {
                ...
                if ($(this).closest('tr').find('td:eq(1)').find('input[type="text"]').val() == "")
                {
                    ...
                    $(this).closest('tr').find('td:eq(1)').find('input[type="text"]').focus();
                    return;
                }
                if ($(this).closest('tr').find('td:eq(2)').find('input[type="text"]').val() == "") {
                    alert("Marks required !");
                    $(this).closest('tr').find('td:eq(2)').find('input[type="text"]').focus();
                    return;
                }
                if ($(this).closest('tr').find('td:eq(3)').find('input[type="text"]').val() == "") {
                    alert("Result required !");
                    $(this).closest('tr').find('td:eq(3)').find('input[type="text"]').focus();
                    return;
                }
    ...
    function savepdfsuccess(result)
    {
        Swal.fire({
            title: 'Successfully save file & data',
            confirmButtonText: 'Ok',
        }).then((result) => {
            $(".savepdf").removeAttr('disabled','disabled');
            $('#tblstudentmark > tbody').html("");
            $(".savepdf").hide();
            count=1;
            window.location.href = url_assement_paper_list;
        });
    }
