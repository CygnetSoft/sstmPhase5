...
function InittblCourses() {
    $('#tblCourses > tbody > tr').on('click', '.btnSubCourse', function (e) {
        ...
        if (Subcount == 4) {
            //localStorage.setItem("url3", $("#ddlCourseStatus").val());
            localStorage.setItem("coursestatus3", document.URL);
        }

        if (Subcount == 5) {
            //localStorage.setItem("url4", $("#ddlCourseStatus").val());
            localStorage.setItem("coursestatus4", document.URL);
        }

        if (Subcount == 6) {
            //localStorage.setItem("url5", $("#ddlCourseStatus").val());
            localStorage.setItem("coursestatus5", document.URL);
        }

        if (Subcount == 7) {
            //localStorage.setItem("url6", $("#ddlCourseStatus").val());
            localStorage.setItem("coursestatus6", document.URL);
        }

        //alert(Subcount);
        //alert($("#ddlCourseStatus").val());
        if (localStorage.getItem("SubfolderCount") > 7) {
            //toastr.error("Maximum Sub Folder Count Reached. You cannot create more than 7 Sub Folders");
            localStorage.setItem("SubfolderCount", 7);
            //return;
        }
        e.preventDefault();
    ...
    $('#tblCourses > tbody > tr').on('click', '.btnTraking', function (e) {//md
        ...
        window.location = '/Course/CourseStatus?Courseid=' + Id + '&MasterCourse=' + $("#MasterCourse").val() + '&MasterCourseId=' + $("#MasterCourseId").val() + '&courseName=' + courseName;
    });
    ...
    if ($("#ddlCourseStatus").val() != 0) //filter with course show in list
    {
            ...
            if (CourseType != "staff") {
                if ($('#ddlCourseStatus').find("option:selected").text() == "Pending")
                    ...
                    aoColumns = [null, { "bVisible": false }, { "bVisible": false }, { "bSortable": false }, null,
                        localStorage.getItem("SubfolderCount") == maximumSubFolderLevel ? { "bVisible": false } : { "bSortable": false },
        ...
        if (userRole === "Administration") {
            var aoColumns = [null, { "bVisible": false }, { "bVisible": false }, { "bSortable": false }, { "bVisible": true },
						localStorage.getItem("SubfolderCount") == maximumSubFolderLevel ? { "bVisible": false } : { "bSortable": false },
        ...
        //    aoColumns = [null, { "bVisible": false }, { "bVisible": false }, null,
